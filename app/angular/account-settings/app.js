	...
	app.register.controller('photoChangeCtrl', ['$scope','$resource','$modalInstance','$upload','localStorageService','shareImg','email','id',
		function($scope,$resource,$modalInstance,$upload,localStorageService,shareImg,email,id){
			...
			$scope.ok = function() {
				...
				console.log(shareImg.imgOrig);
				$scope.upload = $upload.upload({
					url: 'http://localhost:8000/ajax-upload/', //upload.php script, node.js route, or servlet url
		        // method: 'POST' or 'PUT',
		        // headers: {'header-key': 'header-value'},
		        // withCredentials: true,
		        data: {myObj: $scope.myModelObj},
		        file: shareImg.imgOrig, // or list of files: $files for html5 only
