        ...
        function ($scope, $state, $stateParams, $sce) {
            angular.extend($scope, {
                ...
                stateChange: function() {
                    $scope.url = $sce.trustAsResourceUrl('http://store.liveeverfit.com/'  + ($state.current.name=='shop.returns'?'returns':'') + ($state.current.name=='shop.cart'?'cart':'') + ($stateParams.collection?'collections/'+$stateParams.collection:'') + ($stateParams.type?'/'+$stateParams.type:''));
                }
            });
            //init view
